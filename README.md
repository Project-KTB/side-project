...
`유저 관리`
- 사용자는 이메일, 비밀번호, 닉네임, 프로필 이미지 정보를 관리
- 각 유저는 고유한 식별자를 가지며, 이메일과 닉네임에 Unique 제약조건을 적용해 중복 방지

`게시글 관리`
- 사용자가 제목, 내용, 이미지, 작성·수정일시, 조회수, 댓글수, 좋아요수를 포함하는 게시글 관리
- 게시글은 작성자를 참조하고 여러 개의 이미지, 댓글, 좋아요 정보와 관계를 설정
...
`이미지 관리`
- 게시글과 프로필 이미지의 원본 파일명, 저장 파일명, URL, Storage Key, 상태를 관리
